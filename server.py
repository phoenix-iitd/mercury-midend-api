...
async def async_send_message(endpoint: str, data: dict, group_name: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=1000.0) as client:
        ...
        logger.info(
            add_correlation_id(
                f"Message sent to {group_name} ({data['phone']})", "sync", logging.INFO
            )
        )
...
async def revoke_message(message_id: str, phone: str, correlation_id: str) -> bool:
    ...
    logger.debug(
        add_correlation_id(
            f"Sending revoke request to {revoke_url} with payload {payload}",
            correlation_id,
            logging.DEBUG,
        )
    )
    try:
        ...
        logger.debug(
            add_correlation_id(
                f"Revoke request successful for message {message_id}",
                correlation_id,
                logging.DEBUG,
            )
        )
        ...
        logger.error(
            add_correlation_id(
                f"Failed to revoke {message_id} for {phone}: {str(e)}",
                correlation_id,
                logging.ERROR,
            )
        )
...
async def process_revocations(
    request_data: RevokeRequest, correlation_id: str
) -> tuple[int, int, int]:
    """
    ...
    cutoff_time = int(
        (now - datetime.timedelta(hours=request_data.max_age_hours)).timestamp() * 1000
    )
    ...
    try:
        ...
        for doc in simple_docs:
            data = doc.to_dict()
            if (
                data.get("message") == request_data.message_text
                and cutoff_time <= int(data.get("exactTime", "0")) <= current_time
                and data.get("status") != "REVOKED"
            ):
                docs.append(doc)
        logger.debug(
            add_correlation_id(
                f"Found {len(docs)} messages within time window",
                correlation_id,
                logging.DEBUG,
            )
        )
        ...
        logger.error(
            add_correlation_id(
                f"Failed to query Firestore: {str(e)}", correlation_id, logging.ERROR
            )
        )
    ...
    for doc in docs:
        ...
        if not message_id or not phone:
            logger.warning(
                add_correlation_id(
                    f"Missing message_id or phone for doc {doc.id}",
                    correlation_id,
                    logging.WARNING,
                )
            )
        ...
        logger.debug(
            add_correlation_id(
                f"Revoking message {message_id} for phone {phone}",
                correlation_id,
                logging.DEBUG,
            )
        )
        if await revoke_message(message_id, phone, correlation_id):
            ...
            logger.info(
                add_correlation_id(
                    f"Revoked message {message_id} for {phone}",
                    correlation_id,
                    logging.INFO,
                )
            )
        ...
        logger.debug(
            add_correlation_id(
                f"Delaying {delay:.1f}s before next revocation",
                correlation_id,
                logging.DEBUG,
            )
        )
...
async def execute_queue(request_data: QueueRequest, request: Request):
    ...
    logger.info(
        add_correlation_id("Starting queue execution", correlation_id, logging.INFO)
    )
    start = perf_counter()
    success_count = 0
    failed_count = 0
    ...
    rt_queue_ref = create_rt_queue(
        str(int(datetime.datetime.now().timestamp() * 1000)), request_data
    )
    ...
    for idx, group in enumerate(request_data.data, 1):
        try:
            if idx == 1:
                ...
                logger.debug(
                    add_correlation_id(
                        f"Initial delay: {delay:.1f}s", correlation_id, logging.DEBUG
                    )
                )
                await asyncio.sleep(delay)
            logger.info(
                add_correlation_id(
                    f"Processing {idx}/{total} for group {group.name}",
                    correlation_id,
                    logging.INFO,
                )
            )
            ...
            payload = {
                ...
                **(
                    {
                        "caption": request_data.message,
                        "image_url": request_data.filePath,
                    }
                    if request_data.filePath
                    else {"message": request_data.message}
                ),
            }
            ...
            logger.error(
                add_correlation_id(
                    f"Error for {group.name}: {str(e)}", correlation_id, logging.ERROR
                )
            )
        if idx < total:
            ...
            logger.debug(
                add_correlation_id(
                    f"Delay between messages: {delay:.1f}s",
                    correlation_id,
                    logging.DEBUG,
                )
            )
    ...
    logger.info(
        add_correlation_id(
            f"Completed queue in {exec_time:.0f}ms | Success: {success_count}, Failed: {failed_count}",
            correlation_id,
            logging.INFO,
        )
    )
...
async def revoke_queue(request_data: RevokeRequest, request: Request):
    ...
    logger.info(
        add_correlation_id(
            f"Revoking messages for {request_data.user_id}",
            correlation_id,
            logging.INFO,
        )
    )

    revoked_count, failed_count, skipped_count = await process_revocations(
...
async def global_exception_handler(request: Request, exc: Exception):
    logger.error(
        add_correlation_id(
            f"Unhandled exception: {str(exc)}", "NO_CORR_ID", logging.ERROR
        )
    )
    return JSONResponse(
